...
if ( isset( $_POST['opi_buffer_save'] ) && check_admin_referer( 'opi_buffer_settings' ) ) {
    ...
    $message = OPI_Tools::notice( 'success', __( 'Settings saved.', 'opi-buffer' ) );
}

$limits = OPI_Buffer_Settings::get_buffer_limits();
?>
<div class="wrap">
    <h1><?php esc_html_e( 'Buffer Settings', 'opi-buffer' ); ?></h1>
    <a href="<?php echo esc_url( admin_url( 'admin.php?page=opi-buffer' ) ); ?>" class="page-title-action">
        &larr; <?php esc_html_e( 'Back to Queue', 'opi-buffer' ); ?>
    </a>
    ...
    <form method="post">
        ...
        <div class="opi-card">
            <h2 class="opi-section-header"><?php esc_html_e( 'Publishing Rules', 'opi-buffer' ); ?></h2>

            <p class="description" style="margin-bottom:16px;">
                <?php esc_html_e( 'Define how often posts publish based on queue size. The highest threshold you\'ve crossed wins. Rules are sorted automatically.', 'opi-buffer' ); ?>
            </p>
                ...
                <div class="opi-buffer-rule opi-card" style="margin-bottom:12px;">
                    ...
                    <?php if ( $i > 0 ) : ?>
                        <button type="button" class="button opi-buffer-remove-rule">
                            <?php esc_html_e( 'Remove', 'opi-buffer' ); ?>
                        </button>
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
            ...
            <button type="button" id="opi-buffer-add-rule" class="button">
                <?php esc_html_e( '+ Add Rule', 'opi-buffer' ); ?>
            </button>
        </div>

        <?php submit_button( __( 'Save Settings', 'opi-buffer' ), 'primary', 'opi_buffer_save' ); ?>
    </form>
</div>
        ...
        var html =
            '<div class="opi-buffer-rule opi-card" style="margin-bottom:12px;">' +
                ...
                '<div class="opi-form-row">' +
                    ...
                    '</div>' +
                '</div>' +
                '<button type="button" class="button opi-buffer-remove-rule"><?php esc_html_e( 'Remove', 'opi-buffer' ); ?></button>' +
            '</div>';
